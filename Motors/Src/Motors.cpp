...
Motors::Motors(Motor * leftMotor,Motor * rightMotor,Encoder * leftEnc,Encoder * rightEnc) :
	m_leftMotor(leftMotor),m_rightMotor(rightMotor),m_leftEnc(leftEnc),m_rightEnc(rightEnc) {
   	m_position.azimut=m_position.x=m_position.y=0;
}
...
void Motors::computeSpeed() {
	...
	float dAzimutOver2=asinf((rightInc-leftInc)/(2*wheelsSpacing));
	m_position.azimut+=dAzimutOver2; // TODO : ajuster la rotation : pour un tour, azimut indique ~350 pour l'instant
	m_position.x-=dist*sinf(m_position.azimut);
	m_position.y+=dist*cosf(m_position.azimut);
	m_position.azimut+=dAzimutOver2;
}
